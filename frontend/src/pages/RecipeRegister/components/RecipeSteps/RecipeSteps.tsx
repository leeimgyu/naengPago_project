...
const RecipeSteps: React.FC<RecipeStepsProps> = ({ steps, onStepsChange }) => {
  ...
  const handleImageUpload = async (id: number, file: File) => {
    try {
      ...
      onStepsChange(
        steps.map((step) =>
          step.id === id ? { ...step, imageUrl: response.fileUrl } : step
        )
      );
    } catch (error) {
      console.error("조리 이미지 업로드 실패:", error);
      alert("조리 이미지 업로드에 실패했습니다. 다시 시도해주세요.");
      // 에러 발생 시 해당 단계의 이미지 초기화 (선택 사항)
      onStepsChange(
        steps.map((step) =>
          step.id === id ? { ...step, imageUrl: null } : step
        )
      );
    }
  };
    ...
    <section className={styles.section}>
      ...
      <div className={styles.stepsList}>
        {steps.map((step, index) => {
          const imageUrl = step.imageUrl ? `${BACKEND_URL}${step.imageUrl}` : '';
          return (
